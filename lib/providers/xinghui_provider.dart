...
class XingHuiNotifier extends StateNotifier<XingHuiState> {
  ...
  // ====== 模式切换（打字↔语音↔互动）======
  void setMode(XHInteractionMode m) => state = state.copyWith(mode: m);
  ...
  Future<void> setIdentity(XingHuiIdentity i) async {
    ...
    state = state.copyWith(identity: i, lastActionHint: '（身份切换：${i.name}）',
        avatarEmotion: 0);
  }
  Future<void> setLoveMode(XingHuiLoveMode m) async {
    ...
    state = state.copyWith(loveMode: m);
  }
  ...
  Future<void> sendText(String text) async {
    ...
    final userMsg = XHMessageModel(
      ...
      identityContext: state.identity.name,
      modeContext: _modeNames[state.loveMode]!,
      intimacyLevelAtSend: state.intimacyLevel,
    );
    final all = [...state.messages, userMsg];
    state = state.copyWith(messages: all, isTyping: true, avatarEmotion: 3);
    ...
    final resp = _ai.generate(
      trigger: trig, intimacyLevel: state.intimacyLevel,
    ...
    Future.delayed(const Duration(seconds: 3), () {
      if (mounted) state = state.copyWith(clearActionHint: true, avatarEmotion: 0);
    });
  }
  ...
  Future<void> tapPart(XingHuiTrigger part) async {
    ...
    if (part == XingHuiTrigger.tapHand && state.intimacyLevel < 5) {
      final r = _ai.generate(trigger: XingHuiTrigger.tapPalm,
          intimacyLevel: state.intimacyLevel, userNickname: state.userNickname);
      state = state.copyWith(lastActionHint: r.action, avatarEmotion: 2);
      return;
    }
    if (part == XingHuiTrigger.tapHug && state.intimacyLevel < 10) {
    ...
    final resp = _ai.generate(
      trigger: part, intimacyLevel: state.intimacyLevel,
      userNickname: state.userNickname,
    );
    state = state.copyWith(
      lastActionHint: resp.action,
